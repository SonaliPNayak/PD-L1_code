...
EMTscore=(Z_score_ZeB1+Z_score_SLUG-(Z_score_miR200+Z_score_CDH1))./4; 
...
if (EMTscore(200,1)>0.7784)
     count1=count1+1;
     plot(t,EMTscore,'Color','#FF7F7F','Linewidth',1); %M phenotype color orange
     %ylim([-1.2 1])
   
     hold on;
elseif (EMTscore(200,1)>0) && (EMTscore(200,1)<0.2)
     count2=count2+1;
     plot(t,EMTscore,'Color','#FFD17F','Linewidth',1); %H phenotype color yellow
     %ylim([-1.2 1])
     hold on;
elseif (EMTscore(200,1)<-0.2)
     count3=count3+1;
     plot(t,EMTscore,'Color','#7F7F7F','Linewidth',1); %E phenotype color gray
     %ylim([-1.2 1])
     hold on;
%     count_3(k)=count3;
else
